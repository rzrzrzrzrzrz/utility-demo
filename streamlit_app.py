...
def get_api_key():
    ...
    # This is the most robust way to check for a key in Streamlit's st.secrets
    if "GEMINI_API_KEY" in st.secrets:
        return st.secrets["GEMINI_API_KEY"]
    
    # 2. CHECK LOCAL FILE (Development Fallback)
    # Only runs if not on Streamlit Cloud or if the key wasn't in st.secrets
    try:
        # Assuming secrets.toml is in a standard location relative to the script
        with open("st_config/secrets.toml", "r") as f:
            for line in f:
                if "GEMINI_API_KEY" in line:
                    # Robustly extract the value after the '='
                    return line.split("=")[1].strip().replace('"', '').replace("'", "")
    except FileNotFoundError:
        pass # Not found, continue to return None
