...
import urllib.request  # NEW: For downloading the image
...
def tool_vision_scan(image_url, api_key=None):
    ...
    Actually downloads the image and sends it to Gemini Vision if Key is present.
    """
    # 1. Default Demo Result (The "Story" Constraint)
    # We default to X999999 so the "Crossed Meter" demo always works
    result = {
    ...
    if api_key:
        try:
            ...
            # Ask AI to describe it (Proves we "saw" the image)
            # We ask it to verify it IS a meter, but we don't let it override the serial
            # because the Wikipedia image serial won't match our demo database.
            ...
            # Enrich the result with Real AI observation
        ...
        time.sleep(1.0) # Simulate processing time if no key
...
def run_site_flow(user_input, customer, api_key):
    ...
    # 1. Execute Real Vision Tool
    vision_result = tool_vision_scan(demo_image_url, api_key)
    
    # Log the observation to show it's real
