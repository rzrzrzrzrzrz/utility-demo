...
def orchestrator(user_input):
    ...
    1. BUSINESS INTENTS:
       - FINANCE: Bills, debt, charges, refunds, "too high", "balance".
       - TENANCY: Sickness, "can't pay", cutting off, struggling, vulnerable, hospital.
       - SITE: Meter, photo, serial number.
       
    2. GENERAL CHAT:
       - Greetings, small talk, "how are you", thanks.
    ...
    # Fallback Logic (Determinism Safety Net)
    if not decision_json or "error" in decision_json:
        lower_input = user_input.lower()
        if any(x in lower_input for x in ["bill", "high", "cost", "owe", "charge"]): decision_json = {"intent": "FINANCE"}
        elif any(x in lower_input for x in ["pay", "cut", "sick", "dialysis", "help"]): decision_json = {"intent": "TENANCY"}
...
def run_tenancy_flow(user_input, customer, history):
    ...
    # STEP 1: REASON (LLM)
    # Python can't easily link "dialysis" to "medical dependency". LLM does this.
    reasoning_prompt = f"""
    ...
    # STEP 2: ACT (Python)
    # The actual database update is gated by Python logic.
    ...
    prompt = f"""
    ROLE: {db["brand_persona"]}
    DECISION: Vulnerable={decision_json.get('is_vulnerable')}, Reason={decision_json.get('risk_factor')}
    PREVIOUS_STATUS: {flags['is_psr_registered']} (False means you just registered them).
    
    TASK: 
    - Acknowledge the financial stress.
    - IMPORTANT: State that you saw the note about '{decision_json.get('risk_factor')}'.
    - Confirm you have ADDED them to the Priority Services Register (PSR).
    - Reassure them: "Because of this, we cannot and will not disconnect your supply."
    """
    return call_llm_text(prompt, history)
