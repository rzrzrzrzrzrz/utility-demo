...
with st.sidebar:
    ...
    # --- FIXED SIDEBAR BLOCK (NO MORE RANDOM TEXT) ---
    with st.expander("👤 CRM Profile"): 
        # We need to find the fresh profile for Sarah
        profile_view = next((p for p in db["crm"] if p["customer_id"]==current_customer["customer_id"]), {})
        
        # Proper IF/ELSE block to avoid printing DeltaGenerator objects
        if profile_view.get("flags", {}).get("is_psr_registered"):
